...
def get_clip_pred(
    ...
    for b, i in enumerate(slice_positions):
        window_arrays.append(process_window(x[i : i + win_len], sr, audio_settings))
    ...
    if len(result):
        ...
        if mode == "max":
            pred = float(result_array[result_array[:, 1].argmax()][0])
            if label_map is not None:
                pred = label_map[str(int(pred))]
        elif mode == "n_voting":
            pred = int(np.bincount(result_array[:, 0].astype(int)).argmax())
            if label_map is not None:
                pred = label_map[str(int(pred))]
        elif mode == "multi":
            if label_map is not None:
                pred = list(
                    [label_map[str(int(item[0]))], item[1], item[2], item[3]]
                    for item in result_array
                )
            else:
                pred = cast(list[list[float | str]], result_array.tolist())
